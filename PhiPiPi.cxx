...
PhiPiPi::PhiPiPi(const std::string& name, ISvcLocator* pSvcLocator) :
	...
	if ( nt_ana )
		...
		if ( m_tuple_ana) {
			if (m_test4C == 1) {
				...
				status = m_tuple_ana->addItem("mphi_4c",   m_mphi_4c);
			...
			status = m_tuple_ana->addItem("particle_index", m_particle_index, 0, 4);
...
bool PhiPiPi::find_charged_track() {
...
bool PhiPiPi::check_charged_particle() {
	return ((ipGood.size() == 2) && (imGood.size() == 2) && (nCharge == 0));
}

bool PhiPiPi::find_gamma_track_and_assign_4mom() {
	SmartDataPtr<EvtRecEvent> evtRecEvent(eventSvc(), EventModel::EvtRec::EvtRecEvent);
	SmartDataPtr<EvtRecTrackCol> evtRecTrkCol(eventSvc(), EventModel::EvtRec::EvtRecTrackCol);
...
bool PhiPiPi::check_gamma() {
	return m_nGam >= 2;
}

bool PhiPiPi::pid_and_assign_4mom() {
	SmartDataPtr<EvtRecEvent> evtRecEvent(eventSvc(), EventModel::EvtRec::EvtRecEvent);
	SmartDataPtr<EvtRecTrackCol> evtRecTrkCol(eventSvc(), EventModel::EvtRec::EvtRecTrackCol);
...
//			prob_pid.push_back(pid->probPion()); 
			...
			RecMdcKalTrack* mdcKalTrk_pi = (*itTrk)->mdcKalTrack();//After ParticleID, use RecMdcKalTrack substitute RecMdcTrack
...
bool PhiPiPi::check_pid() {
	return (ipip.size() * ipim.size() * iKp.size() * iKm.size() == 1);
}


VertexParameter PhiPiPi::get_vxpar() {
	VertexParameter tmp;
	HepPoint3D vx(0., 0., 0.);
...
bool PhiPiPi::vertex_fit() {
    ...
    if (m_5C_Calibration == 0) {
        ...
        PhiPiPi::calibration(pipTrk, wpip_zHel, 0);
        PhiPiPi::calibration(pimTrk, wpim_zHel, 0);
        PhiPiPi::calibration(KpTrk, wkp_zHel, 1);
        PhiPiPi::calibration(KmTrk, wkm_zHel, 1);
...
//	cout<<"before pphi"<<endl;
	HepLorentzVector pphi = kmfit->pfit(2) + kmfit->pfit(3);
//	cout<<"after pphi"<<endl;
	m_chi1 = kmfit->chisq();
	m_mphi_4c = pphi.m();
//        m_chisq_1g = chisq_1g;
//        m_chisq_3g = chisq_3g;
...
StatusCode PhiPiPi::execute() {
	...
	before_execute();
	if (m_runNo < 0) {
		if (!topology()&false) return StatusCode::FAILURE;
	}
	if (!find_charged_track()) return StatusCode::SUCCESS;
	Ncut1++;
	if (!find_gamma_track_and_assign_4mom()&false) return StatusCode::SUCCESS;
	Ncut2++;
...
void PhiPiPi::corset(HepSymMatrix &V, HepMatrix &C, int n)
{
//cout<<"v="<<V<<endl;
//cout<<"c="<<C<<endl;
  double sum;
...
void PhiPiPi::corgen(HepMatrix &C, HepVector &x, int n)
{
  int i,j;
  int nmax = 100;
...
void PhiPiPi::calibration(RecMdcKalTrack *trk , HepVector &wtrk_zHel, int n )
{
...
//cout<<"wpip_zcal="<<wpip_zcal<<endl;
     ...
     PhiPiPi::corset(wpip_zcal,wpip_zerrc,3);
     HepVector wpip_zgen(3,0);
     PhiPiPi::corgen(wpip_zerrc,wpip_zgen,3);
...
*/
     ...
     PhiPiPi::corset(wpim_zcal,wpim_zerrc,3);
     HepVector wpim_zgen(3,0);
     PhiPiPi::corgen(wpim_zerrc,wpim_zgen,3);
...
*/
     ...
     PhiPiPi::corset(wkp_zcal,wkp_zerrc,3);
     HepVector wkp_zgen(3,0);
     PhiPiPi::corgen(wkp_zerrc,wkp_zgen,3);
...
*/
     ...
     PhiPiPi::corset(wkm_zcal,wkm_zerrc,3);
     HepVector wkm_zgen(3,0);
     PhiPiPi::corgen(wkm_zerrc,wkm_zgen,3);
